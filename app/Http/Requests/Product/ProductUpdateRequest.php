...
class ProductUpdateRequest extends FormRequest
{
    ...
    protected function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $v) {
            ...
            $quantity = $this->input(ProductFieldsEnum::QUANTITY->value);

            // If any of these are null, fallback to existing DB values
            $productId = $this->route('product');
            $product = $productId ? \App\Models\Product::find($productId) : null;
            $batchId = $batchId ?? $product?->milk_batch_id;
            $unitTypeId = $unitTypeId ?? $product?->unit_type_id;
            $quantity = (float)($quantity ?? $product?->quantity ?? 0);
            ...
            if ($batchId && $unitTypeId && $quantity > 0) {
                ...
                if ($unit && $batch) {
                    ...
                    // If editing an existing product on same batch, add back its previous used volume to available before check
                    if ($product && $product->milk_batch_id === $batchId) {
                        $prevUnit = \App\Models\UnitType::find($product->unit_type_id);
                        $prevUsed = (float)($prevUnit?->unit_volume ?? 1) * (float)$product->quantity;
                        $available += $prevUsed;
                    }

                    if ($required > $available) {
                        $v->errors()->add(ProductFieldsEnum::QUANTITY->value, 'Insufficient batch volume for this production update. Required: ' . number_format($required, 3) . ' L, Available: ' . number_format($available, 3) . ' L');
                    }
                }
            }
        });
