...
class OrderService
{
    ...
    public function createForUser(array $payload, int $userId): Order
    {
        try {
            ...
            // Record packaging cost expense per unit sold
            if ($productBuyingSubtotal > 0) {
                Expense::create([
                    'name' => 'Packaging Cost - Order ' . $order->order_number,
                    'description' => 'Auto expense for packaging cost per unit sold',
                    'amount' => $productBuyingSubtotal,
